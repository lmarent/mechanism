...
void auction::reset( auction::configParam_t *params )
{
    ...
    while (params[0].name != NULL) {
	 ...
	 if ( numparams != MOD_REINIT_REQUIRED_PARAMS )
		 throw ProcError("two auction generalized init module - not enought parameters");
...
const char* auction::getModuleInfo( int i )
{
    ...
    switch(i) {
    case auction::I_MODNAME:    return "Two Auction Generalized User procedure";
    ...
    case auction::I_BRIEF:      return "Bidding process for the generalized two auction mechanism";
