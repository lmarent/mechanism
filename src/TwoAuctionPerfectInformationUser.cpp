...
#ifdef DEBUG
	fprintf( stdout,  "two auction perfect information module: start init module \n");
#endif
...
#ifdef DEBUG
	fprintf( stdout,  "two auction perfect information module: end init module \n");
#endif
...
#ifdef DEBUG
	cout << "two auction perfect information module: starting check" << endl;;
#endif
...
#ifdef DEBUG
			fprintf( stdout, "two auction perfect information module: ending check - it does not pass the check, field not parametrized %d.%d \n", 
...
#ifdef DEBUG
				fprintf( stdout, "two auction perfect information module: ending check - it does not pass the check, field not included %d.%d \n", 
							iter->get_eno(), iter->get_ftype());
#endif					
				return 0;
...
#ifdef DEBUG
	fprintf( stdout, "two auction perfect information module: ending check - it pass the check \n");
#endif
...
#ifdef DEBUG
	fprintf( stdout, "two auction perfect information module: start execute with # %d of auctions \n",  (int) auctions->size() );
#endif
...
#ifdef DEBUG
	fprintf( stdout, "two auction perfect information module: in the middle 2 \n");
#endif
...
#ifdef DEBUG
	fprintf( stdout, "two auction perfect information module: end execute \n" );
#endif
...
{
#ifdef DEBUG
	cout <<  "two auction perfect information module: start reset module" << endl;
#endif
    ...
    while (params[0].name != NULL) {
        ...
        if (caseInsensitiveStringCompare(params[0].name, "domainid")) {
			...
			numparams++;
#ifdef DEBUG
		cout << "two auction perfect information module: domainId:" << domainId << endl;
#endif
        }
        params++;
    }
	 ...
	 if ( numparams != MOD_REINIT_REQUIRED_PARAMS )
		 throw ProcError("two auction perfect information init module - not enought parameters");
	
#ifdef DEBUG
	cout << "two auction perfect information module: end reset module" << endl;
#endif
...
const char* auction::getModuleInfo( int i )
{
    ...
    switch(i) {
    case auction::I_MODNAME:    return "Two Auction Perfect Information User procedure";
    case auction::I_ID:		    return "twoauctiongeneralizeduser";
    case auction::I_VERSION:    return "0.1";
    case auction::I_CREATED:    return "2016/03/08";
    case auction::I_MODIFIED:   return "2016/03/08";
    case auction::I_BRIEF:      return "Bidding process for the perfect information two auction mechanism";
...
#ifdef DEBUG
	fprintf( stdout, "two auction perfect information user module: end getModuleInfo \n");
#endif
}
...
{
#ifdef DEBUG
	fprintf( stdout, "two auction perfect information user module: start getErrorMsg \n");
#endif
	
	return NULL;

#ifdef DEBUG
	fprintf( stdout, "two auction perfect information user module: end getErrorMsg \n");
#endif
}
